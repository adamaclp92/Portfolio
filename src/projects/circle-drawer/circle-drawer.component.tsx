...
const CircleDrawer = () => {
  ...
  const undoCoorditaneHandler = () => {
    ...
    setClickCoordinates((prevCoordinates) =>
      prevCoordinates.filter((item) => item != deletingItem)
    );
  ...
  const redoCoordinateHandler = () => {
    ...
    setDeletedCoordinates((prevItems) =>
      prevItems.filter((item) => item != backUpItem)
    );
  ...
  return (
    <React.Fragment>
      ...
      <p>
        Place red circles on the screen. You can undo the last item and redo if
        you have already deleted a circle.
      </p>
      <div className="circledrawer_button_container">
        <button
          disabled={clickCoordinates.length === 0}
          onClick={undoCoorditaneHandler}
        >
          Undo
        </button>
        <button
          disabled={deletedCoordinates.length === 0}
          onClick={redoCoordinateHandler}
        >
          Redo
        </button>
      </div>
      <div className="circledrawer_area" onClick={clickHandler}></div>
      {clickCoordinates.map((coordinate) => (
        <div
          key={coordinate.id}
          className="circledrawer_circle"
          style={{
            position: "absolute",
            left: `${coordinate.x}px`,
            top: `${coordinate.y - 35}px`,
          }}
        ></div>
      ))}
    </React.Fragment>
