...
export default function App() {
  ...
  function toggleDice(die) {
    const newState = numArray.map((num) => {
      return num.key === die.key ? { ...num, isHeld: !num.isHeld } : num;
    });
    setNumArray(newState);
  }
      ...
      <div>
        <main className="tenzies_main">
          ...
          <p className="tenzies_instructions">
            The goal of the game: all the dice should contain the same number.
            You can keep the value by clicking on the dice.
          </p>
          <p className="tenzies_trying">
            Remaining time:{" "}
          ...
          <p className="tenzies_trying">
            Attempts count:{" "}
            <span className="tenzies_trying-count">{tryingCount}/20</span>
          </p>
          <div className="tenzies_contain">
            <div className="tenzies_row">{diceElements}</div>
            {tryingCount < maxCount &&
              (tenzies ? (
                <button
